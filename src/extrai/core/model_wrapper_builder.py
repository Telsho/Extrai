...
class ModelWrapperBuilder:
    ...
    def generate_wrapper_model(self, root_sqlmodel: Type[SQLModel], include_relationships: bool = True) -> Type[BaseModel]:
        """
        ...
        pydantic_model = self._create_pydantic_model_recursive(root_sqlmodel, include_relationships)
        ...
        wrapper_model = create_model(
            wrapper_name,
            entities=(List[pydantic_model], Field(description=f"List of extracted {root_sqlmodel.__name__} entities."))
        )
    ...
    def _enrich_field_description(self, field_info: Any) -> Any:
        ...
        if hasattr(new_field_info, 'max_length') and new_field_info.max_length is not None:
        ...
        if constraints:
            ...
            if new_field_info.description:
                new_field_info.description = f"{new_field_info.description} ({constraint_str})"
            else:
                new_field_info.description = constraint_str
        
        return new_field_info

    def _create_pydantic_model_recursive(self, sql_model: Type[SQLModel], include_relationships: bool = True) -> Type[BaseModel]:
        ...
        if include_relationships:
            ...
            for rel in inspector.relationships:
                if isinstance(rel, RelationshipProperty):
                    ...
                    # Recurse
                    nested_model = self._create_pydantic_model_recursive(target_model, include_relationships)
                    ...
                    relationships[rel.key] = (field_type, Field(default=None, description=field_desc))
