...
class WorkflowOrchestrator:
    ...
    def __init__(
        ...
        self.analytics_collector = analytics_collector or WorkflowAnalyticsCollector(logger=self.logger)
    ...
    async def synthesize(
        ...
        # Hydrate results
        return self.result_processor.hydrate(consensus_results, db_session_for_hydration)
    ...
    async def synthesize_batch(
        ...
        if wait_for_completion:
            return await self.monitor_batch_job(root_batch_id, db_session, poll_interval)
    ...
    async def get_batch_status(self, root_batch_id: str, db_session: Session) -> BatchJobStatus:
    ...
    async def monitor_batch_job(
        ...
        while True:
            ...
            if status in [BatchJobStatus.READY_TO_PROCESS, BatchJobStatus.COUNTING_READY_TO_PROCESS]:
                ...
                elif result.status in [BatchJobStatus.PROCESSING, BatchJobStatus.SUBMITTED]:
                    self.logger.info(f"Intermediate step processed (new status: {result.status}). Continuing workflow...")
                    continue
                ...
                return BatchProcessResult(status=status, message=f"Batch ended with status: {status}")
