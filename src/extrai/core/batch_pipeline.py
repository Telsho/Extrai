...
from .batch_models import BatchJobContext, BatchJobStatus, BatchProcessResult, BatchJobStep
...
class BatchPipeline:
    ...
    async def create_continuation_batch(
        ...
        if "expected_entity_descriptions" in old_context.config:
            new_config["expected_entity_descriptions"] = old_context.config["expected_entity_descriptions"]
    ...
    async def _submit_counting_phase(self, context: BatchJobContext, db_session: Session, step_index: Optional[int] = None):
        input_strings = context.input_strings
        
        # Determine which models to count
        if context.config.get("hierarchical"):
            idx = step_index if step_index is not None else context.config.get("current_model_index", 0)
        ...
        self.logger.info(f"Submitted counting batch for {context.root_batch_id}: {provider_batch_id}")

    async def _submit_extraction_phase(self, context: BatchJobContext, db_session: Session, step_index: int = 0):
        ...
        request = self.request_factory.prepare_request(
            ...
            custom_extraction_process=context.config.get("custom_extraction_process", ""),
            custom_extraction_guidelines=context.config.get("custom_extraction_guidelines", ""),
            custom_final_checklist=context.config.get("custom_final_checklist", ""),
            custom_context=context.config.get("custom_context", ""),
            expected_entity_descriptions=context.config.get("expected_entity_descriptions"),
            previous_entities=previous_entities if previous_entities else None,
            hierarchical_model_index=step_index if context.config.get("hierarchical") else None
        )
        ...
        phase_name = f"step {step_index}" if context.config.get("hierarchical") else "extraction"
        self.logger.info(f"Submitted extraction batch ({phase_name}) for {context.root_batch_id}: {provider_batch_id}")

    async def get_status(self, root_batch_id: str, db_session: Session) -> BatchJobStatus:
    ...
    async def process_batch(
        ...
        return BatchProcessResult(status=status, message="Batch not ready for processing")
        ...
        try:
            ...
            if context.config.get("hierarchical"):
                ...
                if 0 <= current_idx < len(self.model_registry.models):
                    target_model_names = [self.model_registry.models[current_idx].__name__]
                else:
            ...
            for line in results_content.strip().split("\n"):
                ...
                try:
                    ...
                    if content:
                        ...
                        if isinstance(raw_json, list) and raw_json: raw_json = raw_json[0]
                        if isinstance(raw_json, dict):
                            validated_counts = self.entity_counter.validate_counts(raw_json, target_model_names)
            ...
            next_step = context.config.get("current_model_index", 0) if context.config.get("hierarchical") else 0
            await self._submit_extraction_phase(context, db_session, step_index=next_step)
            ...
            return BatchProcessResult(status=BatchJobStatus.FAILED, message=f"Counting failed: {e}")

    async def _process_extraction_completion(self, context: BatchJobContext, db_session: Session) -> BatchProcessResult:
        try:
            ...
            if results:
                ...
                if context.config.get("hierarchical"):
                    return await self._process_hierarchical_step(context, processed, db_session)
            ...
            return await self._handle_batch_retry(context, context.root_batch_id, db_session)
    ...
    async def _process_hierarchical_step(self, context: BatchJobContext, processed_results: List[Dict], db_session: Session) -> BatchProcessResult:
        ...
        if context.config.get("count_entities"):
            await self._submit_counting_phase(context, db_session, step_index=next_index)
            return BatchProcessResult(
            ...
            await self._submit_extraction_phase(context, db_session, step_index=next_index)
            return BatchProcessResult(
                status=BatchJobStatus.PROCESSING,
    ...
    async def _finalize_completion(self, context: BatchJobContext, db_session: Session) -> BatchProcessResult:
        from .result_processor import ResultProcessor
        processor = ResultProcessor(self.model_registry, self.analytics_collector, self.logger)
        ...
        hydrated = processor.hydrate(context.results, db_session, default_model_type=default_model_type)
        return BatchProcessResult(
            status=BatchJobStatus.COMPLETED, 
    ...
    async def _retrieve_and_validate_results(self, context: BatchJobContext) -> List[List[Dict]]:
        client = self.client_rotator.get_next_client()
        results_content = await client.retrieve_batch_results(context.current_batch_id)
        ...
        if self.config.use_structured_output:
            if context.config.get("hierarchical"):
                current_idx = context.config.get("current_model_index", 0)
                if 0 <= current_idx < len(self.model_registry.models):
                    default_model_type = self.model_registry.models[current_idx].__name__
            else:
                default_model_type = self.model_registry.root_model.__name__
    ...
    async def _handle_batch_retry(self, context: BatchJobContext, root_batch_id: str, db_session: Session):
        max_retries = self.config.max_validation_retries_per_revision
        
        if context.retry_count < max_retries:
            context.retry_count += 1
            self.logger.info(f"Retrying batch {root_batch_id} ({context.retry_count}/{max_retries})")
            ...
            if "counting" in context.status.value:
                ...
                current_idx = context.config.get("current_model_index", 0)
                await self._submit_extraction_phase(context, db_session, step_index=current_idx)
        ...
        return BatchProcessResult(status=BatchJobStatus.FAILED, message="Max retries exceeded")
    ...
    def _map_provider_status(self, provider_status) -> BatchJobStatus:
        ...
        elif "process" in status_str or "active" in status_str or "running" in status_str:
