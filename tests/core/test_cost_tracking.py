...
async def test_cost_tracking_generic_openai():
    ...
    client = GenericOpenAIClient(api_key="test", model_name="gpt-4o", base_url="http://test")
...
async def test_structured_cost_tracking():
    ...
    mock_completion = MagicMock()
    mock_completion.choices = [MagicMock(message=MagicMock(parsed={"foo": "bar"}, refusal=None))]
    ...
    # Execute
    await client.generate_structured("system", "user", MagicMock(), analytics_collector=collector)
