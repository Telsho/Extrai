...
class TestExtractionPipeline(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        with patch('extrai.core.extraction_pipeline.ClientRotator'), \
             ...
             patch('extrai.core.extraction_pipeline.ExtractionRequestFactory') as MockFactory:
    ...
    async def test_extract_standard_flow(self):
        self.pipeline.llm_runner.run_extraction_cycle = AsyncMock(return_value=[{"id": 1}])
    ...
    async def test_extract_structured_flow(self):
        ...
        self.pipeline.llm_runner.run_structured_extraction_cycle = AsyncMock(return_value=[{"id": 1}])
        self.pipeline.context_preparer.prepare_example = AsyncMock(return_value="{}")
    ...
    async def test_extract_hierarchical_flow(self):
        ...
        self.pipeline.hierarchical_extractor.extract = AsyncMock(return_value=[{"id": 1}])
    ...
    async def test_count_entities(self):
        self.pipeline.entity_counter.count_entities = AsyncMock(return_value={"RootModel": 10})
        self.pipeline.context_preparer.prepare_example = AsyncMock(return_value="{}")
    ...
    async def test_count_entities_failure(self):
        self.pipeline.entity_counter.count_entities = AsyncMock(side_effect=Exception("Count failed"))
        self.pipeline.context_preparer.prepare_example = AsyncMock(return_value="{}")
        self.pipeline.llm_runner.run_extraction_cycle = AsyncMock(return_value=[])

        await self.pipeline.extract(["doc"], count_entities=True)
        
        self.mock_logger.warning.assert_called_with("Entity counting failed: Count failed")
    ...
    def test_repr(self):
        ...
        self.pipeline.config.use_hierarchical_extraction = True
        repr_str = repr(self.pipeline)
        self.assertEqual(repr_str, "ExtractionPipeline(mode=hierarchical, root=RootModel)")
