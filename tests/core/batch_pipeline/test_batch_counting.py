...
class TestBatchPipelineCounting(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.mock_model_registry.model_map = {"RootModel": self.mock_model_registry.root_model}
        ...
        with patch('extrai.core.batch_pipeline.ClientRotator') as MockClientRotator, \
             patch('extrai.core.batch_pipeline.ExtractionContextPreparer') as MockContextPreparer, \
    ...
    async def test_submit_batch_counting(self):
        # Setup mocks
        self.pipeline.entity_counter.prepare_counting_prompts.return_value = ("count_sys", "count_user")
    ...
    async def test_process_batch_counting_transition(self):
        ...
        mock_client_instance.retrieve_batch_job = AsyncMock(return_value=mock_provider_job)
        
        # Mock counting results
        counting_results_file = '{"id": "line1"}'
        mock_client_instance.retrieve_batch_results = AsyncMock(return_value=counting_results_file)
        mock_client_instance.extract_content_from_batch_response.return_value = '{"RootModel": ["desc1"]}'
        
        self.pipeline.entity_counter.validate_counts.return_value = {"RootModel": ["desc1"]}
        ...
        mock_client_instance.create_batch_job = AsyncMock(return_value=mock_extraction_job)
