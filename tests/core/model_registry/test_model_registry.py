...
class TestModelRegistry(unittest.TestCase):
    ...
    def test_init_success(self):
        with patch('extrai.core.model_registry.SchemaInspector') as MockInspector:
            ...
            mock_inspector.generate_llm_schema_from_models.return_value = '{"type": "object"}'
    ...
    def test_init_discovery_failure(self):
        with patch('extrai.core.model_registry.SchemaInspector') as MockInspector:
            mock_inspector = MockInspector.return_value
            mock_inspector.discover_sqlmodels_from_root.side_effect = Exception("Discovery failed")
    ...
    def test_init_schema_generation_failure(self):
        with patch('extrai.core.model_registry.SchemaInspector') as MockInspector:
            mock_inspector = MockInspector.return_value
            mock_inspector.discover_sqlmodels_from_root.return_value = [MockRootModel]
            mock_inspector.generate_llm_schema_from_models.side_effect = Exception("Schema gen failed")
    ...
    def test_get_schema_for_models(self):
        with patch('extrai.core.model_registry.SchemaInspector') as MockInspector:
            mock_inspector = MockInspector.return_value
            mock_inspector.discover_sqlmodels_from_root.return_value = [MockRootModel]
            mock_inspector.generate_llm_schema_from_models.return_value = '{"full": "schema"}'
            
            registry = ModelRegistry(MockRootModel, self.mock_logger)
            
            # Reset mock to verify specific call
            mock_inspector.generate_llm_schema_from_models.return_value = '{"partial": "schema"}'
            
            schema = registry.get_schema_for_models(["MockRootModel"])
            
            self.assertEqual(schema, '{"partial": "schema"}')
            mock_inspector.generate_llm_schema_from_models.assert_called_with([MockRootModel])
    ...
    def test_init_root_model_class_not_subclass(self):
        ...
        with self.assertRaisesRegex(ConfigurationError, "root_model must be a valid SQLModel class"):
            ModelRegistry(NotAModel, self.mock_logger)
    ...
    def test_init_empty_generated_schema(self):
        with patch('extrai.core.model_registry.SchemaInspector') as MockInspector:
            ...
            with self.assertRaisesRegex(ConfigurationError, "Generated LLM schema is empty"):
                ModelRegistry(MockRootModel, self.mock_logger)
    ...
    def test_get_schema_for_models_exception(self):
        with patch('extrai.core.model_registry.SchemaInspector') as MockInspector:
            ...
            mock_inspector.generate_llm_schema_from_models.side_effect = Exception("Boom")
