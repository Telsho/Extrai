...
def test_hydrate_and_persist_structured_output(session: Session):
    ...
    hydrated_objects = result_processor.hydrate(
        results=mock_consensus_data,
        db_session=session,
        default_model_type="PetConfig"
    )
    ...
    cabin_config = session.query(PetConfig).filter(PetConfig.transport_type == "cabin").one()
    assert cabin_config.max_weight_lbs == 17.64
    
    cargo_config = session.query(PetConfig).filter(PetConfig.transport_type == "cargo").one()
    assert cargo_config.max_weight_lbs == 165.35

    assistance_config = session.query(PetConfig).filter(PetConfig.transport_type == "assistance").one()
