...
def test_pk_collision_prevention():
    ...
    processor = ResultProcessor(
        ...
        analytics_collector=MockAnalytics(),
        logger=logger
    )
    ...
    with Session(engine) as db_session:
        ...
        assert new_parent.id != 1, f"New parent should NOT have ID 1, got {new_parent.id}"
        ...
        assert new_child.parent_id == new_parent.id, f"New child should be linked to new parent {new_parent.id}, got {new_child.parent_id}"
