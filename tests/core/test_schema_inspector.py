...
from sqlalchemy.orm import declarative_base, Mapped, mapped_column, relationship, RelationshipProperty
...
from tests.core.helpers.orchestrator_test_models import (
    Base, Department, Employee, StatusEnum, Project, Member,
    ArticleScenarioModel, TableModel, ModelWithColumnProperty,
    ModelWithNonSerializableColInfo, ModelRelatedToNonSerializable,
    ModelWithNonSerializableRelInfo, ModelWithNonSerializableTableInfo,
    PlainSQLAlchemyModelWithPydanticHints, ModelWithCustomColType,
    FKParent, FKParentDirect, ViewOnlyParent
)
...
@pytest.mark.parametrize("model_cls, checks", [
    (Employee, [
        lambda s: s["comment"] == "Stores detailed information about company employees.",
        lambda s: s["info_dict"] == {"confidentiality": "high"},
        lambda s: s["columns"]["id"]["comment"] == "Unique Employee ID (PK)",
        lambda s: s["columns"]["email"]["info_dict"]["validation_rule"] == "standard_email_format",
        lambda s: s["relationships"]["department"]["info_dict"]["description"] == "The department this employee is assigned to.",
        lambda s: s["relationships"]["department"]["nested_schema"]["table_name"] == "departments",
        lambda s: s["relationships"]["department"]["nested_schema"]["relationships"]["employees"]["nested_schema"]["recursion_detected_for_type"] == "Employee",
        lambda s: "employees.department_id" in s["relationships"]["department"]["foreign_key_constraints_involved"]
    ]),
    (Department, [
        lambda s: s["comment"] == "Stores all company departments.",
        lambda s: s["columns"]["id"]["comment"] == "Unique Department ID (PK)",
        lambda s: s["relationships"]["employees"]["info_dict"]["relationship_detail"] == "All employees belonging to this department.",
        lambda s: s["relationships"]["employees"]["nested_schema"]["table_name"] == "employees",
        lambda s: s["relationships"]["employees"]["nested_schema"]["relationships"]["department"]["nested_schema"]["recursion_detected_for_type"] == "Department"
    ]),
    (Project, [
        lambda s: s["relationships"]["members"]["related_model_name"] == "Member",
        lambda s: s["relationships"]["members"]["secondary_table_name"] == "project_member",
        lambda s: "project_member.project_id" in s["relationships"]["members"]["foreign_key_constraints_involved"]
    ]),
    (TableModel, [
        lambda s: s["table_name"] == "tables",
        lambda s: s["relationships"]["supports"]["nested_schema"]["relationships"]["screws_list"]["nested_schema"]["relationships"]["support"]["nested_schema"]["recursion_detected_for_type"] == "Support"
    ]),
    (ModelWithColumnProperty, [
        lambda s: "data_length" not in s["columns"]
    ]),
    (ModelWithCustomColType, [
        lambda s: s["columns"]["custom_field"]["python_type"] == "unknown_error_accessing_type"
    ]),
    (FKParent, [
        lambda s: "fk_child_sync.parent_id_col" in s["relationships"]["children_sync"]["foreign_key_constraints_involved"]
    ]),
    (FKParentDirect, [
...
def test_relationship_no_fks(inspector, engine):
    ...
    class Src(local_base):
        __tablename__ = "src"
        id: Mapped[int] = mapped_column(primary_key=True)
        rel = relationship("Dest", viewonly=True, primaryjoin="foreign(Src.id)==remote(Dest.id)")
...
@pytest.mark.parametrize("rel_data, expected_id, expected_desc_part", [
    ({"related_model_name": "M", "type": "MANYTOONE", "uselist": False}, "some_rel_ref_id", "_temp_id of the related M"),
    ({"related_model_name": "M", "type": "ONETOMANY", "uselist": True}, "some_rel_ref_ids", "list of _temp_ids for related M"),
    ({"related_model_name": "M", "type": "ONETOMANY"}, None, None) # No uselist
])
def test_process_relationship_for_llm_schema(inspector, rel_data, expected_id, expected_desc_part):
...
@pytest.mark.parametrize("models, check_fn", [
    ([Department], lambda s: "employees_ref_ids" in s["Department"]["fields"]),
    ([Employee, Department], lambda s: "department_ref_id" in s["Employee"]["fields"]),
    ([Project, Member], lambda s: "members_ref_ids" in s["Project"]["fields"] and "projects_ref_ids" in s["Member"]["fields"]),
    ([], lambda s: s == {}),
    ([Employee], lambda s: all(m.value in s["Employee"]["fields"]["status"] for m in StatusEnum)),
    ([ArticleScenarioModel], lambda s: "array[string]" in s["ArticleScenarioModel"]["fields"]["key_topics"].lower()),
    ([PlainSQLAlchemyModelWithPydanticHints], lambda s: s["PlainSQLAlchemyModelWithPydanticHints"]["fields"]["complex_field"].startswith("object //"))
])
...
def test_custom_descriptions_override(inspector):
    ...
    schema = json.loads(inspector.generate_llm_schema_from_models([Employee], custom_field_descriptions=custom))
...
def test_not_a_model_skip(inspector):
    class NotModel: pass
    schema = json.loads(inspector.generate_llm_schema_from_models([Department, NotModel]))
    assert "Department" in schema
    assert "NotModel" not in schema

def test_non_serializable_info(inspector):
    # Column info
    schema_col = json.loads(inspector.generate_llm_schema_from_models([ModelWithNonSerializableColInfo]))
    assert "Info: {" in schema_col["ModelWithNonSerializableColInfo"]["fields"]["data"]

    # Relationship info
    schema_rel = json.loads(inspector.generate_llm_schema_from_models([ModelWithNonSerializableRelInfo, ModelRelatedToNonSerializable]))
    assert "Info: {" in schema_rel["ModelWithNonSerializableRelInfo"]["fields"]["related_ref_id"]

    # Table info
    schema_table = json.loads(inspector.generate_llm_schema_from_models([ModelWithNonSerializableTableInfo]))
...
def test_default_desc_bare_rel(inspector, engine):
    ...
    P.model_rebuild(); C.model_rebuild()
    SQLModel.metadata.create_all(engine)
...
def test_discovery_functions(inspector):
    class NoInspect: pass
    with patch("extrai.core.schema_inspector.inspect", side_effect=NoInspectionAvailable):
        res = []; inspector._collect_all_sqla_models_recursively(NoInspect, res, set())
    ...
    class NoneInspect(Base):
        ...
        id: Mapped[int] = mapped_column(primary_key=True)
    with patch("extrai.core.schema_inspector.inspect", return_value=None):
        res = []; inspector._collect_all_sqla_models_recursively(NoneInspect, res, set())
    ...
    with patch.object(inspector, "_collect_all_sqla_models_recursively", side_effect=Exception):
