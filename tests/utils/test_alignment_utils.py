...
class TestAlignmentUtils(unittest.TestCase):
    ...
    def test_normalize_json_revisions(self):
        ...
        cases = [
            ...
            },
            {
                ...
                "check": lambda res: (
                    len(res) == 2 and
                    res[0][0]['id'] == 1 and res[1][0]['id'] == 1 and
                    res[0][1]['id'] == 2 and res[1][1]['id'] == 2
                )
            },
            {
                ...
                "check": lambda res: (
                    isinstance(res[0], dict) and "results" in res[0] and
                    res[0]["results"][0]["id"] == 1
                )
            },
            {
    ...
    def test_align_entity_arrays(self):
        ...
        cases = [
            ...
            },
            {
                ...
                "check": lambda res: (
                    res[0][0]["name"] == "Alice" and res[1][0]["name"] == "Alice" and
                    res[0][1]["name"] == "Bob" and res[1][1]["name"] == "Bob"
                )
            },
            {
                ...
                "check": lambda res: (
                    res[0][0]["id"] == 1 and res[1][0] is not None
                )
            },
            {
                "name": "deeply nested objects",
                "input": [
                    [{"id": 1, "d": {"n": {"v": "deep"}}}, {"id": 2, "d": {"n": {"v": "deep"}}}],
                    [{"id": 2, "d": {"n": {"v": "deep"}}}, {"id": 1, "d": {"n": {"v": "deep"}}}]
                ],
                "check": lambda res: (
                    res[0][0]["id"] == 1 and res[1][0]["id"] == 1
                )
            ...
            },
            {
                ...
                "check": lambda res: (
                    all(r[0]["name"] == "Alice" for r in res) and
                    all(r[1]["name"] == "Bob" for r in res)
                )
            },
            {
    ...
    def test_calculate_similarity(self):
        ...
        cases = [
            ...
            ("temp id priority", {"_temp_id": "t1", "id": 1}, {"_temp_id": "t1", "id": 2}, 1.0),
            ("empty dicts", {}, {}, 1.0),
            ("no common fields", {"a": 1}, {"b": 2}, 0.0),
            ("partial match", {"a": 1, "b": 2, "c": 3}, {"a": 1, "b": 2, "c": 999}, lambda s: 0.5 < s < 1.0),
    ...
    def test_compare_values(self):
        ...
        for case in cases:
            ...
            with self.subTest(name):
                ...
                if callable(expected):
                    self.assertTrue(expected(score), f"Score {score} failed check for {name}")
                else:
                    self.assertEqual(score, expected, f"Score {score} != {expected} for {name}")
